...
CouplerMeshBackend::CouplerMeshBackend(CouplerMeshEndpoint left,
    ...
    right_overlap_dofs_(CountOverlap(right_))
{
  if (left_overlap_dofs_ == 0 || right_overlap_dofs_ == 0)
    throw std::invalid_argument("CouplerMeshBackend: overlap is empty");
  if (left_overlap_dofs_ == static_cast<std::size_t>(left_.mesh->nverts()) ||
      right_overlap_dofs_ == static_cast<std::size_t>(right_.mesh->nverts())) {
    throw std::invalid_argument(
      "CouplerMeshBackend: overlap must not cover an entire coupler mesh");
  }
}

InterfaceState CouplerMeshBackend::Transfer(Participant& source,
                                            ...
                                            const InterfaceTransfer& transfer)
{
  if (&source == left_.participant && &target == right_.participant) {
    // CouplerParticipant::GetInterface coordinates a PCMS Field receive. No
    // field values are returned through InterfaceState.
    static_cast<void>(source.GetInterface(source_interface));
    left_to_right_.Run();
    return transfer(
      right_.extract_target_interface(*right_.mesh, right_.field.GetField()));
  }
  if (&source == right_.participant && &target == left_.participant) {
    static_cast<void>(source.GetInterface(source_interface));
    right_to_left_.Run();
    return transfer(
      left_.extract_target_interface(*left_.mesh, left_.field.GetField()));
  }
  throw std::invalid_argument(
...
{
  if (!window_active_)
    throw std::logic_error("CouplerMeshBackend: sweep outside a window");
  active_sweep_ = sweep;
}
...
void CouplerMeshBackend::EndSweep(int sweep)
{
  if (active_sweep_ != sweep)
    throw std::logic_error("CouplerMeshBackend: mismatched sweep lifecycle");
  active_sweep_ = -1;
  ++completed_sweeps_;
}
...
Real CouplerMeshBackend::ResidualNorm(std::span<const Real> residual) const
{
  Real local_sum = 0.0;
...
std::size_t CouplerMeshBackend::MeshCount() const noexcept
{
  return 2;
}
  ...
  CouplerMeshEndpoint endpoint)
{
  if (endpoint.participant == nullptr || endpoint.mesh == nullptr)
    throw std::invalid_argument("CouplerMeshBackend: endpoint is incomplete");
...
std::vector<Real> CouplerMeshBackend::CopyField(const Field<Real>& field)
{
...
void CouplerMeshBackend::RestoreField(Field<Real>& field,
...
{
  const Omega_h::HostRead<Omega_h::Real> coordinates(endpoint.mesh->coords());
  std::vector<Real> coordinate(static_cast<std::size_t>(endpoint.mesh->dim()));
  std::size_t count = 0;
  for (int vertex = 0; vertex < endpoint.mesh->nverts(); ++vertex) {
    ...
    count += endpoint.is_overlap(std::span<const Real>(coordinate));
  }
  return count;
}
