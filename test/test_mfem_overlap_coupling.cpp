...
std::unique_ptr<Omega_h::Mesh> ReadCouplerMesh(Omega_h::Library& library,
                                               const std::string& path)
{
  ...
  Log("loaded mesh '" + path + "': dimension=" + std::to_string(mesh->dim()) +
      ", vertices=" + std::to_string(mesh->nverts()) +
      ", elements=" + std::to_string(mesh->nelems()));
  return mesh;
}

tr::InterfaceState ExtractVerticalTrace(const Omega_h::Mesh& mesh,
                                        ...
                                        double x)
{
  ...
  for (int vertex = 0; vertex < mesh.nverts(); ++vertex) {
    if (std::abs(coordinates[mesh.dim() * vertex] - x) <= tolerance) {
      trace.emplace_back(coordinates[mesh.dim() * vertex + 1], values[vertex]);
    }
  }
...
void SetVerticalTrace(const Omega_h::Mesh& mesh, pcms::Field<pcms::Real>& field,
                      double x, const tr::InterfaceState& state)
{
  ...
  for (int vertex = 0; vertex < mesh.nverts(); ++vertex) {
    if (std::abs(coordinates[mesh.dim() * vertex] - x) <= tolerance) {
      trace_vertices.emplace_back(coordinates[mesh.dim() * vertex + 1], vertex);
    }
  }
...
tr::CouplerMeshEndpoint MakeEndpoint(tr::Participant& participant,
                                     std::unique_ptr<Omega_h::Mesh> mesh,
                                     pcms::FieldHandle<pcms::Real> field,
                                     double target_x)
{
  return tr::CouplerMeshEndpoint{
    &participant, std::move(mesh), std::move(field),
...
int main(int argc, char** argv)
{
  ...
  if (argc != 3) {
    if (rank == 0) {
      std::cerr << "Usage: " << argv[0] << " <coupler_A.osh> <coupler_B.osh>\n";
    }
    return EXIT_FAILURE;
  }
  ...
  try {
    Log("starting MFEM overlap-coupling server on " + std::to_string(size) +
        " MPI rank");
    ...
    redev::RCBPtn field_partition(2, field_server_ranks, field_cuts);
    pcms::Coupler field_coupler("mfem_overlap_fields", communicator, true,
    ...
    left_field_app->AddLayout("temperature_volume", left_space.GetLayout());
    ...
    auto right_space = pcms::LagrangeFunctionSpace::FromMesh(
      *right_mesh, 1, 1, pcms::CoordinateSystem::Cartesian, "global",
      pcms::LagrangeFunctionSpace::Backend::OmegaH);
    right_field_app->AddLayout("temperature_volume", right_space.GetLayout());
    auto right_field = right_field_app->AddField(
      "temperature_volume", right_space.CreateField<pcms::Real>());
    Log(
      "registered coupler fields: left owned DOFs=" +
      std::to_string(left_field.GetField().GetLayout().GetNumOwnedDofHolder()) +
      ", right owned DOFs=" +
      std::to_string(
    ...
    tr::CouplerParticipant left(
      *left_field_app, communicator,
      {"left", "temperature_volume", "right_at_0.6", true});
    tr::CouplerParticipant right(
      *right_field_app, communicator,
      {"right", "temperature_volume", "left_at_0.4", true});
    Log("created remote participant GDI controls on the PCMS field "
        "applications; connection is deferred until all channels exist");
    ...
    left.ConfigureFieldExchange(
      {[left_field_app, left_field, &left_receive_count] {
         ++left_receive_count;
         Log("A -> coupler receive #" + std::to_string(left_receive_count) +
             ": begin");
         left_field_app->ReceivePhase([&] { left_field.Receive(); });
         Log("A -> coupler receive #" + std::to_string(left_receive_count) +
             ": complete; field " + FieldSummary(left_field.GetField()));
       },
       [left_field_app, left_field, left_mesh_ptr,
        &left_send_count](const tr::InterfaceState& state) {
         ++left_send_count;
         Log("coupler -> A send #" + std::to_string(left_send_count) +
             ": imposing x=0.6 trace; " + StateSummary(state.View()));
         SetVerticalTrace(*left_mesh_ptr, left_field.GetField(), 0.6, state);
    ...
    right.ConfigureFieldExchange(
      {[right_field_app, right_field, &right_receive_count] {
         ++right_receive_count;
         Log("B -> coupler receive #" + std::to_string(right_receive_count) +
             ": begin");
         right_field_app->ReceivePhase([&] { right_field.Receive(); });
         Log("B -> coupler receive #" + std::to_string(right_receive_count) +
             ": complete; field " + FieldSummary(right_field.GetField()));
       },
       [right_field_app, right_field, right_mesh_ptr,
        &right_send_count](const tr::InterfaceState& state) {
         ++right_send_count;
         Log("coupler -> B send #" + std::to_string(right_send_count) +
             ": imposing x=0.4 trace; " + StateSummary(state.View()));
         SetVerticalTrace(*right_mesh_ptr, right_field.GetField(), 0.4, state);
    ...
    Log(
      "coupling backend ready: meshes=" + std::to_string(backend->MeshCount()) +
      ", A overlap DOFs=" + std::to_string(backend->LeftOverlapDofCount()) +
      ", B overlap DOFs=" + std::to_string(backend->RightOverlapDofCount()));
    ...
    tr::Coupling left_to_right{&left,         "temperature_volume",   &right,
                               "left_at_0.4", tr::IdentityTransfer(), {}};
    left_to_right.backend = backend;
    tr::Coupling right_to_left{&right,         "temperature_volume",   &left,
                               "right_at_0.6", tr::IdentityTransfer(), {}};
    ...
    simulation.SetMonitor([&](const tr::WindowReport& current) {
      report = current;
      std::ostringstream output;
      output << std::setprecision(12) << "accepted window: t=["
             << current.t_start << ", " << current.t_start + current.dt
             << "], attempts=" << current.attempts
             << ", Schwarz iterations=" << current.schwarz_iters
             << ", residual=" << current.schwarz_residual
             << ", converged=" << std::boolalpha << current.converged;
      Log(output.str());
    });
    ...
    passed &=
      Check(backend->MeshCount() == 2 && backend->LeftOverlapDofCount() > 0 &&
    ...
    Log("participant exact-solution errors: A=" +
        std::to_string(left_exact_error) +
        ", B=" + std::to_string(right_exact_error));
    passed &= Check(left_exact_error < 1e-8 && right_exact_error < 1e-8,
                    "coupled MFEM fields reproduce T(x) = 270 + 30x");

    Log(
      "communication totals: A receives=" + std::to_string(left_receive_count) +
      ", A sends=" + std::to_string(left_send_count) +
      ", B receives=" + std::to_string(right_receive_count) +
      ", B sends=" + std::to_string(right_send_count));
    Log("Schwarz summary: iterations=" + std::to_string(report.schwarz_iters) +
        ", residual=" + std::to_string(report.schwarz_residual) +
        ", coupler sweeps=" + std::to_string(backend->CompletedSweeps()));
